...
from active.active_learning import query_selection_adjust_num, analyze
...
parser.add_argument('--rho', type=int, default=0.5, metavar='R',
                    help='sampling rate of the lowest frequency category (default: 0.5)')
parser.add_argument('--lambda_', type=float, default=0, metavar='L',
                    help='lambda value in class-balanced self-training module (default: 0)')
parser.add_argument('--gamma', type=float, default=0.005, metavar='G',
                    help='gamma value in class-balanced self-training module (default: 0.005)')
...
for iDataSet in range(nDataSet):
    ...
    for ep in range(num_epoch):
        if ep in sampling_epochs:
            ...
            idx, already_selected_group = query_selection_adjust_num(feature_extractor,
    ...
    torch.save(feature_extractor.state_dict(),
               'checkpoints/shanghai-hangzhou/' + str(seeds[iDataSet])
               + '_g.pth')
