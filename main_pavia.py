...
parser.add_argument('--gamma', type=float, default=0.03, metavar='G',
                    help='gamma value in class-balanced self-training module (default: 0.03)')
...
sampling_epochs = [40, 45, 50, 55, 60, 65]


def train_without_tlb(data_loader_s, data_loader_t, temp_loader_s, temp_loader_t):
...
for iDataSet in range(nDataSet):
    ...
    for ep in range(num_epoch):
        ...
        if ep <= 39:
            train_without_tlb(train_loader_s, train_loader_t, loader_s_no_shuffle, unlabeled_loader_t)
        elif 39 < ep <= 75:
    ...
    torch.save(feature_extractor.state_dict(),
               'checkpoints/pavia/' + str(seeds[iDataSet])
               + '_g.pth')
